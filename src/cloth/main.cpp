...
static struct {
    ...
    struct {
        b2BodyId pointId[11][11];
        b2JointId horizontal_springId[11][10];
        b2JointId vertical_springId[10][11];
    } d_cloth;
} state;
...
static void init(void) {
    ...
    groundShapeDef.filter.categoryBits = (uint32_t)MyCollisionFilter::Static;
    ...
    shapeDef.filter.categoryBits = (uint32_t)MyCollisionFilter::Dynamic;
    ...
    // cloth mass points
    // b2BodyDef bodyDef = b2DefaultBodyDef();
    b2Vec2 cloth_center = {0.0f, 10.0f};
    b2Vec2 colth_left_upper = {-5.0f, 15.0f};
    b2Circle mass_point_circle = {{0.0f, 0.0f}, 0.4f};
    for (int j = 0; j < 11; j++) {
        for (int i = 0; i < 11; i++) {
            ...
            bodyDef.position = colth_left_upper + b2Vec2(i * 1.0f, j * -1.0f);
            state.d_cloth.pointId[j][i] = b2CreateBody(state.b2d_global_ctx.worldId, &bodyDef);
            ...
            shapeDef.filter.categoryBits = (uint32_t)MyCollisionFilter::Dynamic;
            shapeDef.filter.maskBits = (uint32_t)MyCollisionFilter::Static;
            b2CreateCircleShape(state.d_cloth.pointId[j][i], &shapeDef, &mass_point_circle);
        }
    }
        ...
        // horizontal
        {
            for (int j = 0; j < 11; j++) {
                for (int i = 0; i < 10; i++) {
                    ...
                    joint_def.bodyIdA = state.d_cloth.pointId[j][i];
                    joint_def.bodyIdB = state.d_cloth.pointId[j][i+1];
                    b2Vec2 p1 = b2Body_GetPosition(joint_def.bodyIdA);
                    b2Vec2 p2 = b2Body_GetPosition(joint_def.bodyIdB);
                    joint_def.localAnchorA = b2Body_GetLocalPoint( joint_def.bodyIdA, p1);
                    joint_def.localAnchorB = b2Body_GetLocalPoint( joint_def.bodyIdB, p2);
                    joint_def.length = 1.0f;
                    joint_def.enableSpring = true;
                    joint_def.maxLength = 2.0f;
                    joint_def.minLength = 0.01f;
                    joint_def.enableMotor = true;
                    joint_def.hertz = 4.0f;
                    joint_def.dampingRatio = 0.5f;
                    state.d_cloth.horizontal_springId[j][i] = b2CreateDistanceJoint(state.b2d_global_ctx.worldId, &joint_def);
                }
            }
        }
        ...
        // vertical
        {
            for (int j = 0; j < 10; j++) {
                for (int i = 0; i < 11; i++) {
                    b2DistanceJointDef joint_def = b2DefaultDistanceJointDef();
                    joint_def.bodyIdA = state.d_cloth.pointId[j][i];
                    joint_def.bodyIdB = state.d_cloth.pointId[j+1][i];
                    b2Vec2 p1 = b2Body_GetPosition(joint_def.bodyIdA);
                    b2Vec2 p2 = b2Body_GetPosition(joint_def.bodyIdB);
                    joint_def.localAnchorA = b2Body_GetLocalPoint( joint_def.bodyIdA, p1);
                    joint_def.localAnchorB = b2Body_GetLocalPoint( joint_def.bodyIdB, p2);
                    ...
                    joint_def.hertz = 4.0f;
                    joint_def.dampingRatio = 0.5f;
                    state.d_cloth.vertical_springId[j][i] = b2CreateDistanceJoint(state.b2d_global_ctx.worldId, &joint_def);
                }
            }
        }
    }
        ...
        case SAPP_EVENTTYPE_KEY_DOWN: {
            switch (event->key_code) {
                ...
                case SAPP_KEYCODE_LEFT: {
                    b2WheelJoint_SetMotorSpeed(state.d_car.rearAxleId, 20.0f);
                    b2WheelJoint_SetMotorSpeed(state.d_car.frontAxleId, 20.0f);
...
static void frame(void) {
    ...
    // draw cloth
    {
        ...
        // if (false)
        {
            for (int j = 0; j < 11; j++) {
                for (int i = 0; i < 11; i++) {
                    b2BodyId bid = state.d_cloth.pointId[j][i];
                    b2Vec2 position = b2Body_GetPosition(bid);
                    float angle = b2Rot_GetAngle(b2Body_GetRotation(bid));
                    sgp_push_transform();
                    sgp_translate(position.x, position.y);
                    sgp_rotate(angle);
                    sgp_set_color(1.0f, 0.0f, 0.0f, 1.0f);
                    sgp_draw_outline_circle(0.0f, 0.0f, 0.1f);
                    // sgp_draw_line(0, 0, 0.1f, 0);
                    sgp_pop_transform();
                }
            }
        }
        ...
        // draw spring lines
        {
            // vertical
            {
                for (int j = 0; j < 11; j++) {
                    for (int i = 0; i < 10; i++) {
                        b2BodyId bid1 = state.d_cloth.pointId[j][i];
                        b2BodyId bid2 = state.d_cloth.pointId[j][i+1];
                        b2Vec2 p1 = b2Body_GetPosition(bid1);
                        b2Vec2 p2 = b2Body_GetPosition(bid2);
                        sgp_push_transform();
                        sgp_set_color(0.0f, 0.5f, 0.5f, 1.0f);
                        // sgp_translate(p1.x, p1.y);
                        sgp_draw_line(p1.x, p1.y, p2.x, p2.y);
                        sgp_pop_transform();
                    }
                }
            }
            ...
            // horizontal
            {
                for (int j = 0; j < 10; j++) {
                    for (int i = 0; i < 11; i++) {
                        b2BodyId bid1 = state.d_cloth.pointId[j][i];
                        b2BodyId bid2 = state.d_cloth.pointId[j+1][i];
                        b2Vec2 p1 = b2Body_GetPosition(bid1);
                        b2Vec2 p2 = b2Body_GetPosition(bid2);
                        sgp_push_transform();
                        sgp_set_color(0.5f, 0.5f, 0.0f, 1.0f);
                        // sgp_translate(p1.x, p1.y);
                        sgp_draw_line(p1.x, p1.y, p2.x, p2.y);
                        sgp_pop_transform();
                    }
